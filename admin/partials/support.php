	...
	if ( !empty( $_POST['action'] ) && !empty( $_POST['wow_support_field'] ) ) {
		if (wp_verify_nonce($_POST['wow_support_field'],'wow_support_action') && current_user_can( 'manage_options' )) {
			...
			if ( count($error) == 0 ) {
				...
				$message = sanitize_textarea_field( $_POST['wow-message'] );
				$type = sanitize_text_field( $_POST['wow-message-type'] );
				$headers = array(
...
<div class="wowbox" style="width:100%;">
	<form method="post" action="">
		...
		<div class="wow-admin-col">
			...
			<div class="wow-admin-col-6">
				Plugin:<br/>
				<input type="text" disabled name="wow-plugin" value="<?php if( !empty( $this->name ) ) { echo $this->name . ' v.' . $this->version; };?>">
			</div>
		</div>
		<div class="wow-admin-col">
